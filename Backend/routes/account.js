...
// Route for fetching account information
...
accountRouter.post('/transfer', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        amount = parseFloat(amount.toFixed(2));  // Make sure the amount has two decimal places
        const transaction = await Transaction.create([{
            senderAccountId: senderAccount._id,
            receiverAccountId: receiverAccount._id,
            amount: amount,
            timestamp: new Date()  // Corrected date format
        }], { session: session });
